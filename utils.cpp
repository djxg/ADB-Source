...
static volatile int DBG_FLAG = 0;

void dbg_out1(char* _format, char* parm1)
{
	
	if (!DBG_FLAG) {
		DBG_FLAG = 1;
		FILE* file;
		char buf[1024 * 10], format[64];

		memset(buf, 0x00, sizeof(buf));

		file = fopen("C:\\adb.ini", "a");
		sprintf(format, "%s[%d] : ", __FILE__, __LINE__);
		strcat(format, _format);
		strcat(format, "\r\n");
		sprintf(buf, format, parm1);
